  ...
  return (
    <div className="flex justify-between items-center p-4">
      ...
      <div className="flex items-center space-x-4">
        <div className={`flex rounded-full p-1 ${darkMode ? 'bg-zinc-900' : 'bg-zinc-200'}`}>
          <button 
            className={`px-4 py-1 rounded-full ${activeToggle === 'Forecast' ? 'bg-[#BBD7EC] text-black' : 'text-gray-400'}`} 
            onClick={() => setActiveToggle('Forecast')}
          >
            Forecast
          </button>
          <button 
            className={`px-4 py-1 rounded-full ${activeToggle === 'Air Quality' ? 'bg-[#BBD7EC] text-black' : 'text-gray-400'}`} 
